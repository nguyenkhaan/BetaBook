...
interface ApprovalManagementProps {
  ...
  onUpdateResignation: (id: number, status: 'Chấp nhận' | 'Từ chối', reason?: string) => void;
}

export function ApprovalManagement({ resignations, onUpdateResignation }: ApprovalManagementProps) {
  ...
  const inProgressResignations = resignations.filter(r => r.status === 'Đang xử lý');
  const acceptedResignations = resignations.filter(r => r.status === 'Chấp nhận');
  const deniedResignations = resignations.filter(r => r.status === 'Từ chối');
