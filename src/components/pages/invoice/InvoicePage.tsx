...
import { FileText, Plus, Search, Download, Edit, Trash2, ShoppingCart } from 'lucide-react';
...
const mockDiscountCodes: DiscountCode[] = [
  { id: '1', code: 'SUMMER2026', description: 'Giảm 10% - Khuyến mãi mùa hè', type: 'percentage', value: 10 },
  { id: '2', code: 'NEWCUSTOMER', description: 'Giảm 50,000đ - Khách hàng mới', type: 'fixed', value: 50000 },
  { id: '3', code: 'VIP20', description: 'Giảm 20% - Khách hàng VIP', type: 'percentage', value: 20 },
  { id: '4', code: 'FREESHIP', description: 'Giảm 30,000đ - Miễn phí ship', type: 'fixed', value: 30000 },
  { id: '5', code: 'MEGA50', description: 'Giảm 50% - Siêu khuyến mãi', type: 'percentage', value: 50 },
];
...
export function InvoicePage() {
  ...
  const handleEditInvoice = () => {
    if (selectedInvoice) {
      ...
      setInvoices(
        invoices.map((invoice) => (invoice.id === selectedInvoice.id ? updatedInvoice : invoice))
      );
  ...
  const calculateTotalAmount = () => {
    return createFormData.books.reduce((total, book) => total + book.quantity * book.price, 0);
  };
  ...
  const calculateEditDiscountAmount = () => {
    ...
    const discount = mockDiscountCodes.find(d => d.code === formData.discountCode);
    if (!discount) return 0;

    const subtotal = selectedInvoice.books.reduce((total, book) => total + book.quantity * book.price, 0);
    if (discount.type === 'percentage') {
      return Math.round((subtotal * discount.value) / 100);
    } else {
      return discount.value;
  ...
  const calculateEditSubtotal = () => {
    ...
    return selectedInvoice.books.reduce((total, book) => total + book.quantity * book.price, 0);
  };
  ...
  const calculateTotalItems = () => {
    return createFormData.books.reduce((total, book) => total + book.quantity, 0);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Quản lý hóa đơn bán hàng của Beta Book</p>
        </div>
              ...
              <tr key={invoice.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={() => handleEditInvoiceOpen(invoice)} title="Chỉnh sửa">
                      <Edit className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDeleteInvoiceOpen(invoice)} title="Xóa">
                      <Trash2 className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => openViewBooks(invoice)} title="Xem danh sách sách">
        ...
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader className="border-b pb-4">
            <div className="flex items-start justify-between">
              ...
              {selectedInvoice && (
                <div className={`px-4 py-2 rounded text-sm font-medium ml-4 ${
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="text-gray-900">{selectedInvoice.date} {new Date().toTimeString().slice(0, 8)}</span>
                </div>
                <div>
                ...
                <div className="w-80 space-y-2 text-sm">
                  <div className="flex justify-between border-b pb-2">
                    ...
                    <span className="text-gray-900 font-medium">
                      {selectedInvoice.books.reduce((total, book) => total + book.quantity * book.price, 0).toLocaleString('vi-VN')}
                    </span>
                  </div>
                  {selectedInvoice.discountCode && selectedInvoice.discountAmount && (
                    <div className="flex justify-between border-b pb-2">
                      <span className="font-medium text-orange-600">Mã giảm giá</span>
                      <span className="font-medium text-orange-600">-{selectedInvoice.discountAmount.toLocaleString('vi-VN')}</span>
                    </div>
                  )}
          ...
          <div className="grid gap-3 py-3">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value) => setCreateFormData({ ...createFormData, status: value as Invoice['status'] })}
              >
                <SelectTrigger className="col-span-3">
          ...
          {selectedInvoice && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input value={selectedInvoice.invoiceNumber} readOnly className="col-span-3 bg-gray-50" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="grid grid-cols-4 items-start gap-4">
                ...
                <div className="col-span-3 border border-gray-200 rounded-lg p-3 bg-gray-50">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-200">
                        <th className="text-left py-2 px-2 text-xs font-medium text-gray-500">Tên sách</th>
                        <th className="text-left py-2 px-2 text-xs font-medium text-gray-500">Số lượng</th>
                        <th className="text-left py-2 px-2 text-xs font-medium text-gray-500">Giá</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedInvoice.books.map((book, idx) => (
                        <tr key={idx} className="border-b border-gray-100 last:border-0">
                          ...
                          <td className="py-2 px-2">{book.price.toLocaleString('vi-VN')}đ</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input value={selectedInvoice.items.toString()} readOnly className="col-span-3 bg-gray-50" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Select
                  ...
                  onValueChange={(code) => setFormData({ ...formData, discountCode: code })}
                >
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          {selectedInvoice && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                <Label>Số hóa đơn</Label>
                <Input value={selectedInvoice.invoiceNumber} readOnly className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label>Khách hàng</Label>
                <Input value={selectedInvoice.customer} readOnly className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label>Ngày</Label>
                <Input value={selectedInvoice.date} readOnly className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label>Số mặt hàng</Label>
                <Input value={selectedInvoice.items.toString()} readOnly className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
